...
@
@ This program cycles through the USR LEDs on the BeagleboneBlack Board based on the ON_OFF_FLAG.
@ The Flag is initially zero, but can be changed by a button push on GPIO1_30.
@ The button procedure will cause an interrupt that toggles the flag from 1 to 0 and vice versa.
@ The lights will cycle repeatedly (USR3 -> USR2 -> USR1 -> USR0) as long as the Flag is set to 1.
@ If the flag is changed, the LEDs will turn off after a timer interrupt.
@   The Timer is based on a 32.678 KHz clock and it generates an interrupt every 2 seconds.
@   The Timer also drives the 2 second interval for each light.
@
@ The interrupt table is setup in startup_ARMCA8.s through code composer studio, but there is a manual interrupt hook & chain for necessary cases.
@
...
.equ    CM_PER_GPIO1_CLKCTRL_ADDRESS, 0x44E000AC            @ Address for the GPIO1 clock control register, for turning on GPIO1 and setting up debounce.
...
.equ    THREE_ENABLE_VALUE, 0x03                            @ Value for enabling auto reload in Timer and starting incremental count.
...
.equ    GPIO_IRQSTATUS_0_ADDRESS, 0x4804C02C                @ Address of GPIO_IRQSTATUS_0 register. Used for determining whether the button push triggered the IRQ request and to turn off GPIO1_30 interrupt requests.
   ...
   @ Debounce
.equ    CM_PER_GPIO1_DEBOUNCE_ENABLE, 0x40002               @ Value used to turn on bits 18 and 1 in the CM_PER_GPIO1_CLKCNTRL register.
.equ    GPIO1_DEBOUNCEENABLE_ADDRESS, 0x4804C150            @ Address of the GPIO1 Debounce enable register. Used to set pin for GPIO1 debounce.
.equ    GPIO1_DEBOUNCETIME_ADDRESS, 0x4804C154              @ Address of the GPIO1 Debounce Time register. Used for setting 5ms debouncing on the button.
.equ    DEBOUNCE_REGISTER_VALUE, 0xA0                       @ Value necessary for a 5 ms debounce time.
...
.equ    TIMER7_TCLR_ADDRESS, 0x4804A038                     @ Address of the TIMER7 Control register. Used to start the timer and set auto reload behavior.
...
.equ    INTC_CONFIG_ADDRESS, 0x48200010                     @ Address of the INTC_CONFIG register. Used to reset the INTC.
@       GPIO
...
@ Turns on GPIO1
GPIO_ENABLE:
    LDR R0,=CM_PER_GPIO1_DEBOUNCE_ENABLE                    @ Value to enable clock for a GPIO module and GPIO debounce.
    ...
    @ GPIO1 debouncing
    MOV R0, #BIT_THIRTY                                     @ Loads value for GPIO1_30 pin into R1.
    LDR R1,=GPIO1_DEBOUNCEENABLE_ADDRESS                    @ Loads the value of the debouncable address into R1.
    STR R0, [R1]                                            @ Enable GPIO1_30 debounce.

    MOV R0, #DEBOUNCE_REGISTER_VALUE                        @ Loads the value for the 5ms debounce time into R0.
    LDR R1,=GPIO1_DEBOUNCETIME_ADDRESS                      @ Loads the address for the GPIO1 Debouncing Time register into R1.
    STR R0, [R1]                                            @ Enable GPIO1 debounce for 5ms (same for every pin).
...
INTC_INITIALIZATION:
    ...
    @ If this is used, the SYS_IRQ data location needs to be set at the bottom.
...
TIMER7_ENABLE:
    ...
    @ Starts the count
    MOV R0, #THREE_ENABLE_VALUE                             @ Loads the value for enabling auto reload in timer and timer increment into R0.
    LDR R1,=TIMER7_TCLR_ADDRESS                             @ Loads the address for the TIMER7 control register into R1.
    STR R0, [R1]                                            @ Writes the auto reload and timer start values into the TIMER7 control register.
...
@ Turns off all of the USR LEDs - We need to do this otherwise the wrong LEDs might remain on when we write to the LEDs later
USR_OFF:
    ...
    BEQ NEW_IRQ_ENABLE                                      @ If the flag is not on, return to program loop (LED OFF default).
                                                            @ Else, go to next line.
LED_LOOP:
    @ Checks the state of the LED in the cycle
    LDR R0,=LED_STATE                                       @ Gets the address of the Led state status.
    LDR R1, [R0]                                            @ Loads the value of the state.

    @ Figures out which led to light
    CMP R1, #0x2                                            @ Compares the value to 2.
    BHI USR3_ON                                             @ Jumps to 3 because: >2 means C = 1 & Z = 0.
    BEQ USR2_ON                                             @ Jumps to 2 because: =2 means Z = 1, C = 0.
                                                            @ Else, next line:
    TST R1, #0x1                                            @ Tests for 1.
    BEQ USR0_ON                                             @ Jumps to lighting USR0 if R1 is 0 (z flag is set).
                                                            @ Else, next line:
    B USR1_ON                                               @ Jumps to lighting USR1.

USR3_ON:
    MOV R4, #USR3                                           @ Loads the value to light USR3 into register 4.
    MOV R1, #0x2                                            @ Places 2 into R1 for the next value.
    B   LIGHT_LED                                           @ Jumps to lighting LED.
USR2_ON:
    MOV R4, #USR2                                           @ Loads the value to light USR2 into register 4.
    MOV R1, #0x1                                            @ Places 1 into R1 for the next value.
    B   LIGHT_LED                                           @ Jumps to lighting LED.
USR1_ON:
    MOV R4, #USR1                                           @ Loads the value to light USR1 into register 4.
    MOV R1, #0x0                                            @ Places 0 into R1 for the next value.
    B   LIGHT_LED                                           @ Jumps to lighting LED.
USR0_ON:
    MOV R4, #USR0                                           @ Loads the value to light USR0 into register 4.
    MOV R1, #0x3                                            @ Places 3 into R1 for the next value.
                                                            @ Goes to next line as LED light is right here.
LIGHT_LED:
    STR R1, [R0]                                            @ Stores the next LED value from R1 into the LED state.
...
ON_OFF_FLAG: .WORD  0x0                                     @ Sets aside a byte of memory for the flag. We don't need this much but we got weird stack errors once due to alignment and I don't really want to deal with that right now.
LED_STATE:  .WORD 0x3
